    ...
    definition: (auth, opt) => { // eslint-disable-line no-unused-vars
      const definition = {
        columns: [
          ...
          // entityLocations__columns.latest_inspection__result,
      ...
      definition.ajaxCore = definition.ajaxCore || function (data, callback, settings, queryObject, url, options = {}) {
        ...
        return ajaxes({
          ...
          const filter = locationResponse.value.map(({ id }) => encodeURIComponent(`location eq '${id}'`)).join(' or ');
          const apply = 'groupby((location),aggregate(date with max as latest))';
          ...
          return ajaxes({
            ...
            contentType: 'application/json; charset=utf-8',
            method: 'GET',
            url: `/* @echo C3DATA_LOCATION_NOTES_URL */?$filter=${filter}&$apply=${apply}`
          }).then(({ data: locationNoteAggregateResponse }) => {
            console.log('LOCATION NOTE RESPONSE', locationNoteAggregateResponse);
            ...
            return ajaxes({
              ...
              contentType: 'application/json; charset=utf-8',
              method: 'GET',
              url: `/* @echo C3DATA_LOCATION_INSPECTIONS_URL */?$filter=${filter}`
            });
          }).then(({ data: locationInspectionResponse }) => {
            console.log('LOCATION INSPECTION RESPONSE', locationInspectionResponse);

            return { data: locationResponse };
          });
        }).then(({ data: response }) => {
          callback({
            data: response.value,
