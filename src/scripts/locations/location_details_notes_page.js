...
/* global renderLocationNotesPage__columns renderLocationsPage__views */
...
/* exported renderLocationDetailsNotesPage__currentView */
let renderLocationDetailsNotesPage__currentView;
...
function renderLocationDetailsNotesPage(app, $container, router, auth, opt1, id1, opt2, query) {
  ...
  return auth__checkLogin(auth).then((isLoggedIn) => {
    ...
    return Promise.resolve().then(() => {
      ...
      const definition = renderLocationDetailsNotesPage__views[opt2].definition(opt1, id1);

      const stateSaveWebStorageKey = `locations_${opt1}_${id1}_notes_${opt2}`;
      if (resetState === 'yes' || renderLocationDetailsNotesPage__currentView != opt2) {
